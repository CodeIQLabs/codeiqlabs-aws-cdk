...
- **Patterns Layer Refactoring**

  Introduced a `patterns/` layer that groups pattern-specific stacks and orchestration logic,
  isolating the serverless SaaS pattern so future patterns can be added without cross-pattern
  breakage.

  **New Architecture:**
  - `src/patterns/serverless-saas/orchestrator.ts` - Extracted pattern orchestration from
    ComponentOrchestrator
  - `src/patterns/serverless-saas/stacks/edge/` - Edge stacks (CloudFront, Route53, ACM, WAF, S3,
    SubdomainZone, ApiGatewayDomain)
  - `src/patterns/serverless-saas/stacks/workload/` - Workload stacks (Lambda, DynamoDB, API
    Gateway, EventBridge, ECR, Secrets)

  **Refactored:**
  - `ComponentOrchestrator` reduced to thin dispatcher (~290 lines, down from 1,274)
  - Shared stacks (Organizations, Identity Center, GitHub OIDC) remain in `src/stacks/`
  - Pattern detection delegates to `ServerlessSaasOrchestrator`

  **Deleted (deprecated/unused):**
  - `src/stages/` directory (already empty since v1.6.0)
  - Deprecated customization stacks: InfraAlbStack, AlbDnsRecordStack, ApiGatewayCustomDomainStack,
    ApiGatewayDnsRecordStack, OriginDomainStack, VpcOriginStack, InfraVpcStack
  - Unused domain stacks: DomainDelegationStack, OriginZoneDelegationStack

  **Zero Breaking Changes:**
  - All stack classes remain accessible via `@codeiqlabs/aws-cdk` barrel exports
  - Stack names unchanged (generated by ResourceNaming, not file paths)
  - CloudFormation templates identical before/after
