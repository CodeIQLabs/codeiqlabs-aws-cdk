 ...
 */
export interface DomainDelegationStackProps extends BaseStackProps {
  /** The complete manifest configuration */
  config: UnifiedAppConfig;
}

export class DomainDelegationStack extends BaseStack {
  ...
  constructor(scope: Construct, id: string, props: DomainDelegationStackProps) {
    ...
    // Validate primary domain configuration
    if (!primaryDomain.name) {
      throw new Error('Primary domain is missing required "name" field');
    }
    if (!primaryDomain.hostedZoneId) {
      throw new Error(
        `Primary domain "${primaryDomain.name}" is missing required "hostedZoneId" field`,
      );
    }
  ...
  private createDelegationRecords(primaryDomain: any, delegationProvider: Provider): void {
    // Validate parent hosted zone exists
    if (!this.parentHostedZone) {
      throw new Error('Parent hosted zone is not initialized. Cannot create delegation records.');
    }
    ...
    primaryDomain.delegations.forEach((delegation: any, index: number) => {
      ...
      if (!delegation.subdomain) {
        throw new Error(`Delegation at index ${index} is missing required 'subdomain' field`);
      }
      if (!delegation.targetAccount) {
        throw new Error(`Delegation at index ${index} is missing required 'targetAccount' field`);
      }
      ...
      // Safe to use ! here because we validated parentHostedZone exists above
