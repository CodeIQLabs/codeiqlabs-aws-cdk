 ...
 * - Shared secrets: {project}/{env}/shared/{key}
 * - Per-brand secrets: {project}/{env}/{brand}/{key}
 *
 * @example
 * ```typescript
...
export interface SecretItemConfig {
  /**
   * Secret key - used in the secret name:
   * - Shared: {project}/{env}/{key}
   * - Per-brand: {project}/{env}/{brand}/{key}
   */
  key: string;
  ...
  /**
   * If true, create one secret per brand: {project}/{env}/{brand}/{key}
  ...
  /**
   * Override the global brands list for this specific secret item.
   * When set, only these brands get this secret instead of all brands.
   * Requires perBrand: true.
   */
  ...
  constructor(scope: Construct, id: string, props: SaasSecretsStackProps) {
    ...
    for (const item of items) {
      if (item.perBrand) {
        ...
        const itemBrands = item.brandsOverride ?? brands;
        if (itemBrands.length === 0) {
          throw new Error(
            `Secret "${item.key}" has perBrand: true but no brands are defined in secrets.brands or brandsOverride`,
          );
        }
        ...
        // Sanitize key for construct IDs and export names (replace / with -)
        const sanitizedKey = item.key.replace(/\//g, '-');
        const secret = this.createSecret(
          `${this.toPascalCase(sanitizedKey)}Secret`,
        ...
        // Export secret ARN
        new cdk.CfnOutput(this, `${this.toPascalCase(sanitizedKey)}SecretArn`, {
          value: secret.secretArn,
          exportName: this.naming.exportName(`${sanitizedKey}-secret-arn`),
